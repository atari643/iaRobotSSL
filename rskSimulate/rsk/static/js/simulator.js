function simulator_initialize(backend, isView)
{
    backend.constants(function(constants) {
            ...
            backend.get_state(function(state) {
                ...
                backend.get_wait_ball_position(function(placementCirclePosition){
                    ...
                    if (state.ball != null){
                        ...
                        if (previousBall[0] != state.ball[0] && previousBall[1] != state.ball[1])timeSinceBallMove = Date.now()
                        else if (Date.now() - timeSinceBallMove > 11000){
                            timeSinceBallMove = Date.now()
                            if (state.simulated) backend.teleport("ball", 0, 0, 0)
                        }
                        previousBall = state.ball
                    }
                    if (placementCirclePosition != null){
                        ...
                        //Place the ball automaticaly
                        if (state.simulated  ){
                            backend.teleport("ball", placementCirclePosition[0], placementCirclePosition[1], 0)
                            timeSinceBallMove = Date.now()
                        }
                    }
                    if (placementCirclePosition != null){
        ...
        let timeSinceBallMove = Date.now()
        let previousBall = [0,0]
        let tick = 0
        let T0 = Date.now()
